    ...
    bool shouldDrawButtonAsDown)
{
    ...
    if (auto* pb = dynamic_cast<PowerButton*>(&toggleButton))
    {
        ...
        auto colorRect = toggleButton.getToggleState() ? Colours::dimgrey : Colour(0u, 172u, 1u);
        g.setColour(colorRect);
        g.drawRect(bounds);
...
void PathProducer::process(juce::Rectangle<float> fftBounds, double sampleRate)
{
    ...
    //if there are fft data buffers to pull, if we can pull a buffer, generate the path
    const auto fftSize = leftChannelFFTDataGenerator.getFFTSize();


    //sampleRate/FFT SIZE order  <-- this is the bin width
    //48000/2048 = 23Hz
    ...
    //while there are paths that can be pulled, pull as many as we can to display the most recent path
